...
_commander.default.option(
  "--config [path]",
  "Path to a spack.config.js file to use."
);
...
async function parseSpackArgs(args) {
  ...
  var _config;
  const configOpts = await (0, _spack).compileBundleOptions(
    (_config = opts.config) !== null && _config !== void 0
      ? _config
      : path.resolve("spack.config.js")
  );
  if (opts.entry) {
  ...
  if (!configOpts.output.path) {
    var _outputPath;
    configOpts.output.path =
      (_outputPath = opts.outputPath) !== null && _outputPath !== void 0
        ? _outputPath
        : "[name].js";
  }
  if (!configOpts.output.name) {
    var _output;
    configOpts.output.name =
      (_output = opts.output) !== null && _output !== void 0
        ? _output
        : "[name].js";
  }
  // if (!configOpts.output.name) {
