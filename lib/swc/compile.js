...
function withSourceMap(output, options, destFile, destDir) {
  ...
  output.code += `\n//# sourceMappingURL=${(0, _slash).default(
    (0, _path).relative(destDir, sourceMapPath)
  )}`;
  return {
...
async function compile(filename, opts, sync, outputPath) {
  ...
  try {
    const result = sync
      ? (0, _core).transformFileSync(filename, options)
      : await (0, _core).transformFile(filename, options);
    return result;
  } catch (err) {
