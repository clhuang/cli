...
async function _default({ cliOptions, swcOptions }) {
  ...
  async function output(results) {
    const file =
      ...
      _path.default.basename(cliOptions.outFile || "stdout");
  ...
  async function getProgram(previousResults = new Map()) {
    ...
    for (const filename of await (0, _sources).globSources(
      cliOptions.filenames,
      cliOptions.includeDotfiles
    )) {
      if (
        (0, _sources).isCompilableExtension(filename, cliOptions.extensions)
      ) {
  ...
  async function files() {
    ...
    if (cliOptions.watch) {
      const watcher = await (0, _sources).watchSources(
        cliOptions.filenames,
        cliOptions.includeDotfiles
      );
      watcher.on("ready", () => {
      ...
      for (const type of ["add", "change"]) {
        watcher.on(type, filename => {
          if (
            !(0, _sources).isCompilableExtension(
              filename,
